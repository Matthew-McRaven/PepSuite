...
// There must be two way communication between delta and storage device, since delta must remap contained offsets to
// parent offsets. Storage device must talk to delta, since it must be capable of creating new deltas.
...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>) class Grouped
    : public components::delta::Base<offset_t, val_size_t> {
  public:
    ...
    using any_range =
        boost::any_range<const value_type, boost::forward_traversal_tag, const value_type &, std::ptrdiff_t>;
    struct impl : components::delta::Base<offset_t, val_size_t>::Iterator::AIter {
        impl(const std::vector<delta_t> &deltas, const components::storage::Layered<offset_t, true, val_size_t> *base);
