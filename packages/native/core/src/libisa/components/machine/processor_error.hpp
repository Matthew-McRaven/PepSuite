...
enum class ProcessorErrc {
    ...
    IllegalUnaryInstruction,    // Attempted to decode an opcode as a unary instruction and failed. This is not
                                // recoverable.
    IllegalNonunaryInstruction, // Attempted to decode an opcode as a nonunary instruction and failed. This is not
                                // recoverable.
    IllegalAddressingMode, // Attempted to execute an instruction with an invalid addressing mode. Must not crash, but
                           // should terminate simulation.
};

// To synthesise a custom status code domain for `ProcessorErrc`, inject the following
// template specialisation:
SYSTEM_ERROR2_NAMESPACE_BEGIN
template <> struct quick_status_code_from_enum<ProcessorErrc> : quick_status_code_from_enum_defaults<ProcessorErrc> {
