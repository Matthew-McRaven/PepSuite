...
masm::driver<address_size_t, stage_t>::assemble_os(project_t &project, std::shared_ptr<masm::project::source_file> os,
...
bool masm::driver<address_size_t, stage_t>::register_transform(
    ...
    if (transforms_.find(stage) != transforms_.end())
        return false;
...
std::pair<bool, std::string> masm::driver<address_size_t, stage_t>::do_loop(project_t &project, stage_t target_stage) {
        ...
        if (auto transform = transforms_.find(stage); transform == transforms_.end()) {
            ...
            if (!stage_success) {
                ...
                for (auto item : new_work) {
                    if (work_queue_.find(item.first) == work_queue_.end())
                        work_queue_[item.first] = {};
    ...
    if (failed_stage)
        return {false, fmt::format(";ERROR: Transform failed for stage {}.", magic_enum::enum_name(target_stage))};
    return {true, ""};
}