...
void masm::elf::addr_line_mapping(std::ostream &os, const masm::elf::top_level_section<addr_size_t> &section) {
    ...
    enumerator = [&](const masm::elf::code_section<addr_size_t> &section) -> void {
        for (auto line : section.body_ir->ir_lines) {
            if (line->object_code_bytes() == 0)
                continue;
            if (auto as_macro = std::dynamic_pointer_cast<const masm::ir::macro_invocation<addr_size_t>>(line);
                as_macro) {
                auto as_code = std::static_pointer_cast<const masm::elf::code_section<addr_size_t>>(as_macro->macro);
                enumerator(*as_code);
            } else {
                auto item =
                    AddressLineRegion<addr_size_t>{line->base_address(), line->end_address(), line->listing_line};
    ...
    for (auto &[prefix, section] : sections) {
        ...
        for (auto symbol : symbols) {
            ...
            if (symbol->binding == symbol::binding_t::kGlobal)
                binding = STB_GLOBAL;
            else if (symbol->binding == symbol::binding_t::kImported)
                binding = STB_WEAK;
        ...
        // MUST SAVE STR!! Failing to save str will cause it to immediately expire, and the c_str will immediately be
        // deleted.
