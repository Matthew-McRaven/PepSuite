...
std::shared_ptr<const masm::Macro> masm::macro_registry::macro(const std::string &macro_name) const {
    if (auto v = _registry.find(macro_name); v != _registry.end())
        return v->second;
    return nullptr;
}
...
bool masm::macro_registry::register_nonunary_system_call(const std::string &macro_name,
                                                         const std::string &template_text) {
    std::string macro_text = fmt::vformat(template_text, fmt::make_format_args(macro_name));
    return register_macro(macro_name, macro_text, MacroType::SystemMacro);
}

bool masm::macro_registry::register_macro(const std::string &macro_name, const std::string &macro_text,
