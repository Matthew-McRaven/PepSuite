...
void masm::message_handler<address_size_t>::log_message(
    std::shared_ptr<masm::elf::code_section<address_size_t>> section, uint32_t line, message message) {
...
std::list<std::tuple<uint32_t /*line*/, masm::message>> masm::message_handler<address_size_t>::errors_for_section(
    ...
    if (recurse && section->body_preprocess) {
        ...
        for (auto [line, macro] : section->body_preprocess->macro_line_mapping) {
            ...
            for (auto &[local_line, local_message] : local_errors)
                local_line = line;
        ...
        auto errors = messages_ |
                      ...
                      boost::adaptors::transformed([](auto &line) {
                          return ret_t{std::get<1>(line), std::get<2>(line)};
                      });
