    ...
    transform_t tx_sanity1 = [=](project_t &proj, std::list<driver_t::work_t> &work) {
        ...
        std::transform(work.begin(), work.end(), std::back_inserter(result_work), [&](auto &value) {
            ...
            if (parsed_tls.find(tls) == parsed_tls.end()) {
                ...
                if (tls.second)
                    val &= whole_program_sanity_fixup<uint16_t>(proj, tls.second);
...
std::tuple<bool, std::shared_ptr<masm::project::project<uint16_t>>> asmb::pep10::driver::assemble(std::string user_text,
                                                                                                  std::string os_text) {
    using namespace asmb::pep10::driver;
