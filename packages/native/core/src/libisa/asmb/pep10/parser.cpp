...
auto asmb::pep10::parser::parse(std::shared_ptr<masm::project::project<uint16_t>> &project,
    ...
    for (auto [index, line] : section->body_token.value().tokens | boost::adaptors::indexed(0)) {
        ...
        // TODO: Fix case sensitivity on dot commands.
            ...
            if (boost::iequals(text_dot, "ASCII"))
                ...
                std::tie(local_success, local_message, local_line) =
                    parse_SCALL(start, last, symbol_table, project->macro_registry);
                if (local_success && local_symbol) {
                    local_success = false;
                ...
                std::tie(local_success, local_message, local_line) =
                    parse_USCALL(start, last, symbol_table, project->macro_registry);
                if (local_success && local_symbol) {
                    local_success = false;
        ...
        // We had an error, let's try and provide helpful messages for what went wrong.
            ...
            // Unexpected end of line.
                ...
                project->message_resolver->log_message(section, index,
