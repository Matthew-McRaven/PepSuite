...
std::string error_name(assemble_error_code code) { return std::string(magic_enum::enum_name(code)); }
...
bool set_user_program(std::shared_ptr<project> &project, emscripten::val maybe_text) {
    if (!maybe_text.isString())
        return false;
    else
        project->user = maybe_text.as<std::string>();
    return true;
}
...
bool set_os(std::shared_ptr<project> &project, emscripten::val maybe_text) {
    if (!maybe_text.isString())
        ...
        project->os = maybe_text.as<std::string>();
    return true;
}
...
assemble_error_code assemble(std::shared_ptr<project> project) {
    if (project->os.empty()) {
        ...
        if (!lookup.has_value())
            return assemble_error_code::kUnableToLookupOS;
...
emscripten::val os_listing(std::shared_ptr<project> project) {
    if (!project->masm_project->image->os)
        return emscripten::val::undefined();
...
std::shared_ptr<simulator> make_simulator() { return std::make_shared<simulator>(simulator{}); }

void set_program_image(std::shared_ptr<simulator> simulator,
                       std::shared_ptr<masm::elf::AnnotatedImage<uint16_t>> machine_image) {
...
emscripten::val run(std::shared_ptr<simulator> simulator) {
    auto loaded = isa::pep10::load_user_program(simulator->machine_image->image(), simulator->machine,
                                                isa::pep10::Loader::kDiskIn);
    if (loaded.has_error())
        return emscripten::val::undefined();
    ...
    if (!ret.has_value())
        ...
        return emscripten::val(ret.value());
}

void begin_simulation(std::shared_ptr<simulator> simulator) { simulator->machine->begin_simulation(); }

void end_simulation(std::shared_ptr<simulator> simulator) { simulator->machine->end_simulation(); }

EMSCRIPTEN_BINDINGS(pep10) {

    emscripten::function("errorName", &error_name);
    emscripten::enum_<element_type>("ElementType").value("Pep", element_type::kPep);
    ...
    emscripten::class_<macro>("Macro").property("name", &macro::name).property("text", &macro::text);
    ...
    emscripten::class_<std::shared_ptr<simulator>>("Pep10Simulator")
        ...
        // Returns a component of the Pep10 Computer by name.
        // For Pep/10, valid names are "cpu" and "mem"
        .function("getComponent", dummy);

    // Needed for `getComponent`
    emscripten::class_<std::shared_ptr<int>>("CPU")
        .constructor(nullptr)
        .function("readRegister", dummy)
        .function("writeRegister", dummy)
        .function("registerCount", dummy)
        .function("readCSR", dummy)
        .function("writeCSR", dummy)
        .function("readPackedCSR", dummy)
        .function("writePackedCSR", dummy)
        .function("CSRCount", dummy)
        .function("cycleCount", dummy)
        .function("instructionCount", dummy);
    emscripten::class_<std::shared_ptr<void *>>("MainMemory")
        .function("get", dummy)
        .function("set", dummy)
        .function("read", dummy)
