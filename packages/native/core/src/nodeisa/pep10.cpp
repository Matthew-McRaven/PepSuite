...
// Get listing of user.text as peph.
emscripten::val user_peph(std::shared_ptr<project> project) {
    if (!project->masm_project->image->user)
        return emscripten::val::undefined();
    auto string = masm::utils::generate_pretty_object_code(project->masm_project->image->user, 16, true);
    return emscripten::val(string);
}

// Get listing of user.text as pepb.
emscripten::val user_pepb(std::shared_ptr<project> project) {
    if (!project->masm_project->image->user)
        return emscripten::val::undefined();
    auto string = masm::utils::generate_pretty_object_code(project->masm_project->image->user, 2, false);
    return emscripten::val(string);
}
...
EMSCRIPTEN_BINDINGS(pep10) {
    ...
    emscripten::class_<std::shared_ptr<project>>("AssemblyProject")
        ...
        .function("formattedPeph", &user_peph)
        .function("formattedPepb", &user_pepb)
