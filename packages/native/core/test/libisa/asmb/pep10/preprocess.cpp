...
TEST_CASE("Recognize existing macros", "[asmb::pep10::preprocessor]") {
        ...
        for (auto value : work) {
            ...
            for (auto child : local_children) {
                result_work.emplace_back(driver_t::work_iterable_t::value_type{
                    stage_t::RAW, std::static_pointer_cast<masm::elf::code_section<uint16_t>>(child)});
            }
    ...
    SECTION("No such macro.") {
        ...
        CHECK(std::get<1>(*errors.begin()).message ==
              fmt::vformat(masm::frontend::detail::error_does_not_exist, fmt::make_format_args("HELLO1")));
    }
    ...
    SECTION("Invoke 1-arity macro with 2 args.") {
        ...
        REQUIRE(errors.size() == 1);
        CHECK(std::get<1>(*errors.begin()).message ==
              fmt::vformat(masm::frontend::detail::error_bad_arg_count, fmt::make_format_args(2, 1)));
    }
    ...
    SECTION("Invoke 1-arity macro with 0 args.") {
        ...
        REQUIRE(errors.size() == 1);
        CHECK(std::get<1>(*errors.begin()).message ==
              fmt::vformat(masm::frontend::detail::error_bad_arg_count, fmt::make_format_args(0, 1)));
    }
