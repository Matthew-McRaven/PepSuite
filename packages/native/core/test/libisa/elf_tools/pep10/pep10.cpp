...
TEST_CASE("Convert ELF image to Pep/10 machine", "[elf_tools::pep10]") {
    ...
    SECTION("Assemble OS successfully.") { REQUIRE(os_to_image() != nullptr); }
    ...
    SECTION("Buffer user program to diskIn.") {
        ...
        REQUIRE(machine_result.has_value());
        auto machine = machine_result.value();
        assert(isa::pep10::load_user_program(image->image(), machine, isa::pep10::Loader::kDiskIn));
    ...
    SECTION("Buffer user program to RAM.") {
        ...
        REQUIRE(machine_result.has_value());
        auto machine = machine_result.value();
        assert(isa::pep10::load_user_program(image->image(), machine, isa::pep10::Loader::kRAM));
