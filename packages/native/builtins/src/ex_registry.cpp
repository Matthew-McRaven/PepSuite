...
// Figure
figure::figure(std::string proc, uint16_t chapter, std::string fig, std::vector<test> tests,
...
std::optional<figure> registry::find(std::string proc, uint16_t chapter, std::string figure) const {
    auto match = [=](const struct figure &fig) {
        return fig.proc == proc && fig.chapter == chapter && fig.fig == figure;
    };
    if (auto val = std::find_if(_figures.begin(), _figures.end(), match); val != _figures.end())
        return *val;
    else
        return std::nullopt;
}

const std::vector<figure> &registry::figures() const { return _figures; }

void registry::add_macro(macro macro) { _macros.emplace_back(macro); }

std::optional<macro> registry::find(std::string macro_name) const {
    auto match = [=](const macro &mac) { return mac.name == macro_name; };
    if (auto val = std::find_if(_macros.begin(), _macros.end(), match); val != _macros.end())
        return *val;
    else
        return std::nullopt;
}

const std::vector<macro> &registry::macros() const { return _macros; }
