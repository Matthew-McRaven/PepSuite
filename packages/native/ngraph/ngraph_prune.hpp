...
template <class T>
const typename NGraph::tGraph<T>::vertex &sole_in_neighbor(const NGraph::tGraph<T> &A,
                                                           const typename NGraph::tGraph<T>::vertex &v) {
...
template <class T> bool terminal_node(typename NGraph::tGraph<T>::const_iterator p) {
    return (NGraph::tGraph<T>::out_neighbors(p).size() == 0 && NGraph::tGraph<T>::in_neighbors(p).size() != 0);
}
template <class T> bool leaf_node(typename NGraph::tGraph<T>::const_iterator p) {
    return NGraph::tGraph<T>::out_neighbors(p).size() == 0;
}

template <class T> bool reflexive_node(typename NGraph::tGraph<T>::const_iterator p) {
    return (NGraph::Graph::in_neighbors(p) == NGraph::Graph::out_neighbors(p));
}

template <class T> bool u_turn_node(typename NGraph::tGraph<T>::const_iterator p) {
...
template <class T> bool single_incoming_edge(typename NGraph::tGraph<T>::const_iterator p) {
...
NGraph::tGraph<T> reduceList(const NGraph::tGraph<T> &input,
    ...
    while (previousSize != currentSize) {
        ...
        for (typename NGraph::tGraph<T>::const_iterator p = workingCopy.begin(); p != workingCopy.end(); p++) {
            if (leaf_node<T>(p)) {
...
// Returns the the graph formed by recursively removing all verticies with out degree 0.
// Only works for directed graphs. Is equivalent to calcuating the 1-core of the graph.
template <class T> NGraph::tGraph<T> reduce(const NGraph::tGraph<T> &input) {
