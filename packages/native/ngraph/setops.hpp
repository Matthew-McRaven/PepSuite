...
template <class T> bool operator==(const std::set<T> &A, const std::set<T> &B) {
...
template <class T> std::set<T> &operator+=(std::set<T> &A, const std::set<T> &B) {
...
template <class T> std::set<T> operator-(const std::set<T> &A, const std::set<T> &B) {
    ...
    std::set_difference(A.begin(), A.end(), B.begin(), B.end(), inserter(res, res.begin()));
...
template <class T> std::set<T> symm_diff(const std::set<T> &A, const std::set<T> &B) {
    ...
    std::set_symmetric_difference(A.begin(), A.end(), B.begin(), B.end(), inserter(res, res.begin()));
...
template <class T, class constT> inline bool includes_elm(const std::set<T> &A, constT &a) {
...
//
template <class T> int intersection_size(const std::set<T> &A, const std::set<T> &B) {
    int res = 0;

    typename std::set<T>::const_iterator first1 = A.begin(), last1 = A.end(), first2 = B.begin(), last2 = B.end();
...
//
template <class T> int big_small_intersection_size(const std::set<T> &A, const std::set<T> &B) {
    int res = 0;
    typename std::set<T>::const_iterator first = B.begin(), last = B.end();
    for (; first != last; first++) {
        if (includes_elm(A, *first))
            res++;
    }
    return res;
}
...
// needless copying of elements.)
//
template <class T> int union_size(const std::set<T> &A, const std::set<T> &B) {
    int res = 0;
...
template <class T> int set_difference_size(const std::set<T> &A, const std::set<T> &B) {
