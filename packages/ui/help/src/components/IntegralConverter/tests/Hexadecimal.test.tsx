...
describe('1-byte Hexadecimal <IntegralConverter />', () => {
	...
	it('has been mounted', () => {
		const component = shallow(<IntegralConverter byteLength={1} state={state} setState={setState} base={16} />);
		expect(component.length).toBe(1);
	});
	// Default to 0 when only given prefix
	state = 255
	it('defaults to 0 ', () => {
		const wrapper = shallow(<IntegralConverter byteLength={1} state={state} setState={setState} base={16} />);
...
describe('2-byte Hexadecimal <IntegralConverter />', () => {
	...
	const setState = (newState: number) => { state = newState; };
	it('has been mounted', () => {
		const component = shallow(<IntegralConverter byteLength={2} state={state} setState={setState} base={16} />);
		expect(component.length).toBe(1);
	});
	// Default to 0 when only given prefix
	state = 255
	it('defaults to 0 ', () => {
		const wrapper = shallow(<IntegralConverter byteLength={2} state={state} setState={setState} base={16} />);
		const input = wrapper.find("input");
		input.simulate('change', { currentTarget: { value: '0x' } });
		expect(state).toBe(0)
	});
