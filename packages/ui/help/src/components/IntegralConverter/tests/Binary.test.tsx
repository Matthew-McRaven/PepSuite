...
describe('Binary <IntegralConverter />', () => {
	...
	it('has been mounted', () => {
		const component = shallow(<IntegralConverter byteLength={1} state={state} setState={setState} base={2} />);
		expect(component.length).toBe(1);
	});
	// Default to 0 when only given prefix
	state = 255
	it('defaults to 0 ', () => {
		const wrapper = shallow(<IntegralConverter byteLength={1} state={state} setState={setState} base={2} />);
