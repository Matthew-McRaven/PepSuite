...
export function chunk<T>(array: T[], chunkSize: number) {
  ...
  const chunkedArray: Array<Array<T>> = Array()
...
export const parseObjectCode: ParseSignature = (maybeCode: string) => {
  ...
  if (!/[zZ][zZ]/.exec(maybeBytes[maybeBytes.length - 1])) return { success: false, errorMessage: "Object code string must terminate in ZZ." }
