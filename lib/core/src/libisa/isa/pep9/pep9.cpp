...
std::array<addr_map, 256> pep9_init_mmap() {
    ...
    for (int it = 0; it < 256; it++) {
        using tp =
            const std::pair<const isa::pep9::instruction_mnemonic, std::shared_ptr<isa::pep9::instruction_definition>>;
...
isa_definition::isa_definition() : isa(isa_map), riproll(pep9_init_mmap()) {}

const isa_definition &isa_definition::get_definition() { return definition; }

std::string isa::pep9::as_string(instruction_mnemonic mnemon) { return std::string(magic_enum::enum_name(mnemon)); }
...
bool isa::pep9::is_opcode_unary(instruction_mnemonic mnemon) { return definition.isa.at(mnemon)->is_unary; }
