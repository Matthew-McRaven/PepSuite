...
auto masm::frontend::preprocessor<address_size_t, tokenizer_t>::preprocess(
    ...
    if (out_graph.num_nodes() != 0) {
        ...
        project->message_resolver->log_message(parent, parent_line,
                                               {masm::message_type::kError, detail::error_circular_include});
        return {false, {}};
    }
    ...
    for (auto [line_it, line] : section->body_token.value().tokens | boost::adaptors::indexed(0)) {
        ...
        if (!valid_invoke) {
            project->message_resolver->log_message(section, line_it,
                                                   {masm::message_type::kError, detail::error_invalid_macro});
    ...
    for (auto [line, macro_name, macro_args] : potential_invocations) {
        if (!registry->contains(macro_name)) {
            ...
            project->message_resolver->log_message(
                section, line,
                {masm::message_type::kError,
                 fmt::vformat(detail::error_does_not_exist, fmt::make_format_args(macro_name))});
