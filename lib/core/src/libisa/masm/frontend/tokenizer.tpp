...
masm::frontend::tokenize_result<typename tokenizer_t::token_type> masm::frontend::tokenize(std::string input,
...
std::tuple<bool, token_enum_t, std::string>
masm::frontend::match(typename tokenized_line_t<token_enum_t>::iterator &first,
    ...
    if (first == end)
        return ret;
    else if (auto val = set.find(first->first); val != set.end()) {
...
template <typename token_enum_t>
std::tuple<bool, token_enum_t, std::string>
masm::frontend::match_not(typename tokenized_line_t<token_enum_t>::iterator &first,
                          const typename tokenized_line_t<token_enum_t>::iterator &end,
                          const std::set<masm::frontend::token_type> set, bool advance_iterator) {
    std::tuple<bool, token_enum_t, std::string> ret = {false, {}, {}};
    if (first == end)
