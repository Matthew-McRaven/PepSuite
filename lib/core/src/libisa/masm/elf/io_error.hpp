...
template <> struct quick_status_code_from_enum<ElfMMIOErrc> : quick_status_code_from_enum_defaults<ElfMMIOErrc> {
    ...
    static const std::initializer_list<mapping> &value_mappings() {
        static const std::initializer_list<mapping> v = {
            ...
            {ElfMMIOErrc::BadIOType,
             "MMIO Definition contained illegal type not in: {kInput, kOutput}",
             {errc::invalid_argument}},
            {ElfMMIOErrc::BadByteCount,
             "Serialized definition array must be a multiple of 4 bytes.",
             {errc::illegal_byte_sequence}},
            {ElfMMIOErrc::NoSuchIndex,
             "MMI Definition symtab index does not refer to a valid symbol.",
             {errc::invalid_argument}},
        };
        return v;
    }
