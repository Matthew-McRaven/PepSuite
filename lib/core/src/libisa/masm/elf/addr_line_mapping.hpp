...
template <typename addr_size_t> struct AddressLineRegion {
    addr_size_t start_address, end_address;
    std::size_t line_number;
    bool contains_address(addr_size_t addr) const { return start_address <= addr && addr <= end_address; }
    bool contains_line(std::size_t line) const { return line_number == line; }

  private:
    friend class cereal::access;
    template <class Archive> void serialize(Archive &ar) { ar(start_address, end_address, line_number); }
};

// Struct to represent a collection of address<=>line mappings, since I can't serialize a vector directly.
// It is designed to be serialized easily using ceral.
// Take care that you pass the right address type, or parsing will probably implode.
template <typename addr_size_t> struct AddressLineRegionVector {
    ...
    bool contains_address(addr_size_t addr) const {
        for (const auto &item : vec)
            if (item.contains_address(addr))
                return true;
        return false;
    }
    bool contains_line(std::size_t line) const {
        for (const auto &item : vec)
            if (item.contains_line(line))
                return true;
        return false;
    }
  ...
  private:
    friend class cereal::access;
    template <class Archive> void serialize(Archive &ar) { ar(vec); }
};
} // namespace masm::elf