...
// Must be included before io_error, or compilinh fails
#include "mmio.hpp"
...
result<std::vector<masm::elf::mmio::Definition>> masm::elf::mmio::from_bytes(const std::vector<uint8_t> &bytes) {
    ...
    if (bytes.size() % 4 != 0)
        return status_code(ElfMMIOErrc::BadByteCount);
    uint16_t type, index;
    ...
    for (size_t it = 0; it < bytes.size(); it += 4) {
        ...
        if (type_enum != Type::kInput && type_enum != Type::kOutput)
            return status_code(ElfMMIOErrc::BadIOType);
        ...
        if (index == 0)
            return status_code(ElfMMIOErrc::NoSuchIndex);
