 ...
 */
template <typename address_size_t, typename stage_t> class driver {
  public:
     ...
     * Sections at the lowest stage are pulled from the queue, and the driver searches for a transformation matching
     * that stage. If a transformation, the section will be run through the transformer and re-queued on the work queue
     * with the section's new stage. Execution continues until all sections in the project reach the `target_stage.`
     */
    std::pair<bool, std::string> assemble_os(project_t &project, std::shared_ptr<masm::project::source_file> os,
                                             stage_t target_stage);
