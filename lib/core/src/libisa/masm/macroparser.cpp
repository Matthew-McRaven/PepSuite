...
template <typename Iterator> struct macro_parser : qi::grammar<Iterator, macro(), ascii::space_type> {
    macro_parser() : macro_parser::base_type(start) {
        ...
        start %= qi::no_skip['@'] >> qi::lexeme[+(qi::char_ - ascii::space)] >> int_;
    }
...
std::tuple<bool, std::string, uint8_t> masm::analyze_macro_definition(std::string macro_text) {
    ...
    bool r = boost::spirit::qi::phrase_parse(cbegin, first_line.cend(), g, boost::spirit::ascii::space, macro);
