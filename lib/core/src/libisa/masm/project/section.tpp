...
masm::elf::top_level_section<address_size_t>::add_or_get_invoke_class(
    ...
    if (auto as_macro = std::dynamic_pointer_cast<masm::elf::macro_subsection<address_size_t>>(section);
        as_macro != nullptr) {

        for (auto &[key, value] : macro_def_to_invoke_class) {
            auto &[name, args] = key;
            if (name != as_macro->header.name)
                continue;
            else if (args != as_macro->macro_args)
                continue;
            else
                return value;
        };
    ...
    else if (auto as_tls = std::dynamic_pointer_cast<masm::elf::top_level_section<address_size_t>>(section);
             as_tls != nullptr) {
        if (!invoke_dependency_graph.includes_vertex(0))
            invoke_dependency_graph.insert_vertex(0);
        return 0;
    } else
...
std::shared_ptr<masm::elf::macro_subsection<address_size_t>> masm::elf::top_level_section<address_size_t>::insert_macro(
    std::shared_ptr<code_section<address_size_t>> direct_parent, uint32_t line_number, std::string macro_name,
    std::vector<std::string> macro_args, std::string macro_text) {
    ...
    if (auto as_macro = std::dynamic_pointer_cast<masm::elf::macro_subsection<address_size_t>>(direct_parent);
        as_macro != nullptr) {
        child_macro->containing_section = as_macro->containing_section;
    }
    // Otherwise, we have a top level section, and it is the containing top level section.
    else if (auto as_tls = std::dynamic_pointer_cast<masm::elf::top_level_section<address_size_t>>(direct_parent);
             as_tls != nullptr) {
    ...
    for (auto &line : child_macro->body_raw.value().lines)
        line.append("\n");
    ...
    if (!direct_parent->body_preprocess)
        direct_parent->body_preprocess = masm::elf::code::pre<address_size_t>();
