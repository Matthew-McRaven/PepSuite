...
enum class MachineErrc {
    ...
    NoSuchDevice // There is no memory mapped device with a given name. It is up to the user to determine if this is
                 // recoverable.
...
template <> struct quick_status_code_from_enum<MachineErrc> : quick_status_code_from_enum_defaults<MachineErrc> {
