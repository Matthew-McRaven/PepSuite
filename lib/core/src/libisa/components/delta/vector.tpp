...
requires(
    components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>) template <class Iter>
components::delta::Vector<offset_t, val_size_t>::template impl<Iter>::impl(
    Iter iter, const components::storage::Base<offset_t, true, val_size_t> *base)
...
requires(
    components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>) template <class Iter>
std::unique_ptr<typename components::delta::Base<offset_t, val_size_t>::Iterator::AIter> components::delta::Vector<
    offset_t, val_size_t>::template impl<Iter>::clone() const {
...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    components::delta::Vector<offset_t, val_size_t>::Vector(
        components::storage::Base<offset_t, true, val_size_t> &storage)
...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<
         val_size_t>) void components::delta::Vector<offset_t, val_size_t>::apply_forward() {
    if (_deltas.empty())
        return;
    // Set implicitly bypasses deltas, so we do not need to worry about applying our delta itself generating deltas.
    // Call .value() to force resolution of possible errors. Perform no error handling, because we can't recover from a
    // bad delta.
    for (const auto &[offset, _, new_value] : _deltas)
        this->_storage.set(offset, new_value).value();
}

template <typename offset_t, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<
         val_size_t>) void components::delta::Vector<offset_t, val_size_t>::apply_backward() {
    if (_deltas.empty())
        return;
    // Set implicitly bypasses deltas, so we do not need to worry about applying our delta itself generating deltas.
    // Call .value() to force resolution of possible errors. Perform no error handling, because we can't recover from a
    // bad delta.
    for (const auto &[offset, old_value, _] : _deltas)
        this->_storage.set(offset, old_value).value();
}

template <typename offset_t, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&
             components::storage::Integral<val_size_t>) void components::delta::Vector<offset_t, val_size_t>::clear() {
    _deltas.clear();
}
...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    ...
    components::delta::Vector<offset_t, val_size_t>::cbegin() {
...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    typename components::delta::Base<offset_t, val_size_t>::Iterator
    components::delta::Vector<offset_t, val_size_t>::cend() {
...
template <typename offset_t, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<
         val_size_t>) void components::delta::Vector<offset_t, val_size_t>::add_delta(offset_t offset,
                                                                                      val_size_t old_value,
