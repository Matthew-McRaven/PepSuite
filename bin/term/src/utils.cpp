...
bool is_resource(const std::string &path) { return static_cast<bool>(parse_file_as_resource(path)); }
...
std::optional<std::string> read_figure(uint16_t chapter, const std::string &figure, element_type type) {
    ...
    if (!fig)
        return std::nullopt;
    if (auto it = fig->elements.find(type); it == fig->elements.end())
        return std::nullopt;
    else
        return it->second;
}
...
std::string read_file_or_resource(const std::string &path) {
    if (auto as_rc = parse_file_as_resource(path); as_rc) {
        ...
        if (book_fig)
            ...
            throw std::logic_error(fmt::format("No such Figure rc://{}.{}.{}", proc, ch, fig));
    } else {
...
std::string read_default_os() { return *read_figure(9, "00", element_type::kPep); }

void write_errors(void *errors, void *error_file) { throw std::logic_error("Not yet implemented"); }