...
class TermFormatter : public CLI::Formatter {
  public:
    TermFormatter(std::map<std::string, std::map<std::string, std::string>> &param_names,
        ...
        : CLI::Formatter(), param_names(param_names), detailed_descriptions(detailed_descriptions) {}
...
std::string TermFormatter::make_heirarchical_usage(const CLI::App *app, std::string name,
    ...
    if (!options.empty()) {
        ...
        for (auto option : options) {
            ...
            if (!subcommand_heirarchy.empty() && option->get_name(false, false) == "--help-all")
                continue;
...
std::string TermFormatter::make_options_names(const CLI::Option *opt, std::string group_name) const {
    ...
    if (auto str = find_param_name(opt, group_name); !str.empty()) {
...
std::string TermFormatter::find_param_name(const CLI::Option *opt, std::string group_name) const {
    ...
    if (auto map_it = param_names.find(group_name); map_it != param_names.end()) {
        auto map = map_it->second;
        for (auto name : option_names) {
            if (auto tag_it = map.find(name); tag_it != map.end()) {
