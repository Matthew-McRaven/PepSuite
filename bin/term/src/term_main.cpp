...
int main(int argc, char *argv[]) {
    ...
    auto version_flag = parser.add_flag(
        "-v,--version", [&](int64_t flag) { handle_version(values, flag); }, version_string);

    std::string about_string = "Display information about licensing, Qt, and developers.";
    auto about_flag = parser.add_flag(
        "--about", [&](int64_t flag) { handle_about(values, flag); }, about_string);
    ...
    auto cpuasm_d2_flag = cpuasm_subcommand->add_flag("--d2", [&](int64_t){handle_databus_size(values, true);},
    cpu_2byte);
    ...
    // Add flags to select 1-byte or 2-byte CPU data bus.
    auto cpurun_d2_flag = cpurun_subcommand->add_flag("--d2", [&](int64_t){handle_databus_size(values, true);},
    cpu_2byte_run);
    // Allow full control section to be enabled iff 2-byte data bus is enabled.
    //auto cpurun_full_ctrl_flag = cpurun_subcommand->add_flag("--full-control",[&](int64_t){handle_full_control(values,
    true);}, cpu_full_control);
    //cpurun_full_ctrl_flag->needs(cpurun_d2_flag);
    // Maximum number of cycles to be executed.
    //std::string max_cycles_text =
    QString::fromStdString(microMaxStepText).arg(BoundExecMicroCpu::getDefaultMaxCycles()).toStdString();
    //cpurun_subcommand->add_option("-m", values.m,
    max_cycles_text)->expected(1)->needs(cpurun_full_ctrl_flag)->check(CLI::PositiveNumber)
    ...
    */
    ...
    try {
        ...
        if (values.had_about || values.had_version)
            return 0;
...
void handle_figure(command_line_values &values) {
    ...
    std::cout << fmt::format("Computer Systems, 6th edition.\nFigure {}.{}", values.ch, values.fig) << std::endl
              << std::endl;
    if (!figure)
        std::cout << "Figure not found!" << std::endl;
    else
...
void handle_asm(command_line_values &values) {
    ...
    if (values.enable_elf)
        project->as_elf->image().save(elf_file);
}
...
void handle_run(command_line_values &values) {
    ...
    } else if (!values.obj.empty())
        throw std::logic_error("--obj is not yet implemented.");
    else
        reader.load(values.elf);
    ...
    if (values.had_echo_output)
        std::cout << std::endl;
    if (!values.o.empty())
        save_runtime_output(bytes_output, {values.o});
...
void handle_asm(command_line_values &values)
{
        ...
        if(!sourceFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
                ...
                QObject::connect(helper, &ASMBuildHelper::finished, QCoreApplication::instance(),
...
void handle_run(command_line_values &values)
{
        ...
        ASMRunHelper *helper = new ASMRunHelper(objText, stepMaxValue, textOutputFileName,
                                                                        textInputFileName,
*AsmProgramManager::getInstance()); helper->set_echo_charout(values.had_echo_output); QObject::connect(helper,
&ASMRunHelper::finished, QCoreApplication::instance(), &QCoreApplication::quit);
...
void handle_cpuasm(command_line_values &values, QRunnable **runnable)
{
        ...
        if(!sourceFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
                ...
                QObject::connect(helper, &CPUBuildHelper::finished, QCoreApplication::instance(),
&QCoreApplication::quit);
                (*runnable) = helper;
        }
...
preconditionText, nullptr); if(!values.e.empty()) { helper->set_error_file(QString::fromStdString(values.e));
                }

                QObject::connect(helper, &CPURunHelper::finished, QCoreApplication::instance(),
&QCoreApplication::quit);
                (*run) = helper;
        }
        else {
                ...
                QObject::connect(helper, &MicroStepHelper::finished, QCoreApplication::instance(),
&QCoreApplication::quit);
                (*run) = helper;
